...
export class Signal<Type extends Record<string, any> = any> {
  ...
  constructor(initial: Type, props?: { persistName?: string | undefined }) {
    ...
    if (props && props.persistName) {
      ...
      if (key && key !== "undefined") {
        ...
        this.data = Object.assign(initial, restored);
      }
    }
    ...
    for (const key in this.data) {
      ...
      // ? soft set on this.data so no event is called
  ...
  private publish<T extends keyof Type>(events: T[]) {
    ...
    if (this.pn) {
      localStorage.setItem(this.pn, JSON.stringify(this.data));
    }
  }
   ...
   */
  set(data: Type) {
    Object.assign((this.data as any).$_internal_data, data);
    ...
    if (isComp) {
      ...
      const listenerRaw = () => {
        if (!document.body.contains(listenerRaw.element)) {
          ...
          this.subscribers[eventName as keyof Type | "__ALL__"] = this
            .subscribers[eventName as keyof Type].filter(
      ...
      if (!this.subscribers[eventName as keyof Type | "__ALL__"]) {
        this.subscribers[eventName as keyof Type | "__ALL__"] = [];
      }
      listenerRaw.idx =
        this.subscribers[eventName as keyof Type | "__ALL__"].length;
      this.subscribers[eventName as keyof Type | "__ALL__"].push(listenerRaw);
      return el;
    }
    this.subscribers[eventName as keyof Type | "__ALL__"].push(listener!);
    return undefined;
  }
  /**
   *  Cradova Signal
  ...
  get pass(): Record<keyof Type, [string, Signal<any>]> {
    ...
    const keys = Object.keys(this.data) as (keyof Type)[];
    this.passers = {} as any;
